...
async function runConcurrencyStressTest() {
  ...
  await pool.query(
    `INSERT INTO public.account_room_limits (account_id, plan_id)
     VALUES ($1, 'free')
     ON CONFLICT (account_id) DO UPDATE SET plan_id = 'free', override_total_rooms = NULL, override_watch_rooms = NULL, override_permanent_rooms = NULL, override_room_duration_hours = NULL`,
    [accountId]
  );
  ...
  if (entA.plan_id !== "free" || entA.max_total_rooms !== 5 || entA.max_room_duration_hours !== 24) {
