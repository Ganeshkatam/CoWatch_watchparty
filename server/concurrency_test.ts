...
async function runConcurrencyStressTest() {
  ...
  let testRoomsA: string[] = [];
  let testRoomsB: string[] = [];
  let testRoomsC: string[] = [];
  let singleUser = "";
  ...
  try {
    ...
    testUserIds = profileRes.rows.map(r => r.id);
    singleUser = testUserIds[0];
    ...
    console.log("Cleaning up Test 12 fixtures and restoring user entitlements...");
    try {
      await pool.query("DELETE FROM public.vbrowser_reservations WHERE pool_id = $1", [P7A_POOL]);
      for (let i = 0; i < testRoomsA.length; i++) {
        if (testUserIds[i] && testRoomsA[i]) {
          await pool.query("SELECT public.delete_room_authoritative($1, $2)", [testUserIds[i], testRoomsA[i]]).catch(() => {});
        }
      }
      for (const rId of testRoomsB) {
        if (singleUser && rId) {
          await pool.query("SELECT public.delete_room_authoritative($1, $2)", [singleUser, rId]).catch(() => {});
        }
      }
      for (let i = 0; i < testRoomsC.length; i++) {
        if (testUserIds[i] && testRoomsC[i]) {
          await pool.query("SELECT public.delete_room_authoritative($1, $2)", [testUserIds[i], testRoomsC[i]]).catch(() => {});
        }
      }
