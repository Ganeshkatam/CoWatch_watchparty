...
export function sanitizeServerUserMessage(raw: string | undefined | null): UserMessage {
  ...
  const technicalTermsPattern =
    /\b(postgres|postgresql|redis|zincrby|socket\.io|webrtc|peerconnection|candidate|ice|operationid|uncaught|syntaxerror|typeerror|nullpointer|internalerror|exception|cluster\s+node|econnreset|etimedout|econnrefused|websocket|handshake|gateway)\b/i;
  const sqlPattern =
